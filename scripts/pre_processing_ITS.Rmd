...
tree <- read_qza(paste0(path, "/Analysis/phylogeny/seqs_nocontam_ITS_aligned_masked_tree_rooted.qza"))
...
feature_tree <- sapply(tree2$data$tip.label, 
                       grep, 
                       rownames(tax_table))
tree2$data$tip.label <- tax_table[feature_tree, 9]
```
...
write.csv(tax_table, 
          file = paste0(path_out, "/231202_Sugi_phyllosphere_June_ITS/processed_data/tax_table_ITS.csv"))
write.csv(tax_table2, 
          file = paste0(path_out, "/231202_Sugi_phyllosphere_June_ITS/processed_data/tax_table2_ITS.csv"))
write.csv(otus_data2, 
          file = paste0(path_out, "/231202_Sugi_phyllosphere_June_ITS/processed_data/otus_data_ITS.csv"), 
          row.names = as.character(rownames(otus_data2)))
save(list = c("tree2", "tree_rarefied2"), 
     file = paste0(path_out, "/231202_Sugi_phyllosphere_June_ITS/processed_data/trees.Rdata"))
```
