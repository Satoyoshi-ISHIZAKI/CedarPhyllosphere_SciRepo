...
```{r}
plot_fapro <- plot_top_taxa(ps_fapro_res_mod, 
...
```{r}
dist_bray <- vegdist(t(otu_table(ps_fapro_res_mod)), method = "bray")

# exclude functionally Unassigned taxa for NMDS
ps_fapro_res_mod_xUA <- subset_taxa(ps_fapro_res_mod, group != "Unassigned")
dist_bray_xUA <- vegdist(t(otu_table(ps_fapro_res_mod_xUA)), method = "bray")
```
...
```{r}
set.seed(123)
fit_FG <- envfit(nmds_fapro$nmds, t(otu_table(subset_taxa(ps_fapro_res_mod, taxa_sums(ps_fapro_res_mod) > 0))))
fit_FG$tax_table <- tax_table(ps_fapro_res_mod)
show(fit_FG)
df_fit_FG <-data.frame(tax_table(ps_fapro_res_mod)[rownames(fit_FG$vectors$arrows), "group"], 
...
set.seed(123)
fit_FG_xUA <- envfit(nmds_fapro_xUA$nmds, t(otu_table(subset_taxa(ps_fapro_res_mod_xUA, 
                                                                  taxa_sums(ps_fapro_res_mod_xUA) > 0))))
fit_FG_xUA$tax_table <- tax_table(ps_fapro_res_mod_xUA)
show(fit_FG_xUA)
df_fit_FG_xUA <-data.frame(tax_table(ps_fapro_res_mod_xUA)[rownames(fit_FG_xUA$vectors$arrows), "group"], 
