...
BVOC_2306_az040 <- BVOC_2306 %>% subset(!(treeID %in% c("AJ_002_01", "AZ_004_01")))
BVOC_2306_log_az040 <- BVOC_2306_log %>% subset(!(treeID %in% c("AJ_002_01", "AZ_004_01")))
...
BVOC_2306_az040 <- BVOC_2306_az040[, -which(colnames(BVOC_2306_az040) == "Longifolene")]
BVOC_2306_log_az040 <- BVOC_2306_log_az040[, -which(colnames(BVOC_2306_log_az040) == "Longifolene")]
...
BVOC_2306_log_az_040_rm <- assay(BVOC_2306_log_az040)[, -bCary] #remove beta-Caryophyllene
```
...
#with AZ_040_1
#calculate log-transformed total BVOC- and MTs emission
BVOC_2306_total_log_az040 <- data.frame(total = log(rowSums(assay(BVOC_2306_az040)), base = 10)) %>%
  rownames_to_column(var = "sampleID")
MT_2306_log_az040 <- data.frame(MTs = log(rowSums(assay(BVOC_2306_az040[, BVOC_2306_az040$mol_weight == "Mono"])) + 1, base = 10)) %>%
  rownames_to_column(var = "sampleID")
SQT_2306_log_az040 <- data.frame(SQT = log(rowSums(assay(BVOC_2306_az040[, BVOC_2306_az040$mol_weight == "Sesqui"])) + 1, base = 10)) %>%
  rownames_to_column(var = "sampleID")
DT_2306_log_az040 <- data.frame(DT = log(rowSums(assay(BVOC_2306_az040[, BVOC_2306_az040$mol_weight == "Di"])) + 1, base = 10)) %>%
  rownames_to_column(var = "sampleID")
