...
save(nmds_bray, file = paste0(path_out, "/231202_Sugi_phyllosphere_June_16S/processed_data/BVOC/NMDS_bray_16S_BVOC2306_K.Rdata"))
```

##envfit

```{r envfit}
load(paste0(path_out, "/231202_Sugi_phyllosphere_June_16S/processed_data/BVOC/NMDS_bray_16S_BVOC2306_K.Rdata"))

#extract the samples for which both microbial and BVOC data are available
mat_16S <- nmds_bray$nmds$points
mat_16S_BVOC <- mat_16S[match(all_2306_log$sampleID, rownames(mat_16S)), ]

#check if the samplename in the bacterial data and the BVOC data match
all(rownames(mat_16S_BVOC) == all_2306_log$sampleID)
```

```{r envfit}
set.seed(45)
fit_bray <- envfit(mat_16S_BVOC, 
...
save(fit_bray, file = paste0(path_out, "/231202_Sugi_phyllosphere_June_16S/analysis/envfit_bray_BVOC_LODrev_K.Rdata"))
write.csv(df_fit_bray, file = paste0(path_out, "/231202_Sugi_phyllosphere_June_16S/analysis/envfit_bray_BVOC_LODrev_K.csv"))
```
...
show(df_fit)
write.csv(df_fit, file = paste0(path_out, "/231202_Sugi_phyllosphere_June_16S/analysis/envfit_bray_BVOC_LODrev_omitLOD_K.csv"))
```

```{r}
df_nmds <- data.frame(mat_16S_BVOC) %>% 
  rownames_to_column(var = "sampleID") %>%
...
p_bray <- ggplot(df_nmds, mapping = aes(x = MDS1, y = MDS2)) +
  ...
  geom_text(data = bray_signif,  
            aes(x = MDS1, y = MDS2 + 0.1, label = rowname), 
            size = 2) +
...
show(p_bray)
#ggsave(plot = p_bray, filename = paste0(path_out, #"/231202_Sugi_phyllosphere_June_16S/figures/envfit_BVOC_log_LODrev_K.png"), 
