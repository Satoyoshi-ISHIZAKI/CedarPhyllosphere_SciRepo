...
```{r}
plot(log(apply(otu_table(ps_Phylum) / rowSums(otu_table(ps_Phylum)), 2, mean)), 
     apply(otu_table(ps_Phylum), 2, occurrence), 
     xlab = "log(Mean Relative Abundance)", 
     ylab = "Occupancy")
abline(h = 2, col = "red")

#cut off Classes whose occurrence is less than 2
Phylum_abundant <- otu_table(ps_Phylum)[, which(apply(otu_table(ps_Phylum), 2, occurrence) > 1)]
```
...
```{r}
set.seed(56)
fit_ASV <- envfit(nmds_bray$nmds, Phylum_abundant)
fit_ASV$tax_table <- tax_table(ps_Phylum)
show(fit_ASV)
df_fit_ASV <-data.frame(tax_table(ps_Phylum)[rownames(fit_ASV$vectors$arrows), "Phylum"], 
                        fit_ASV$vectors$arrows, 
                        r = fit_ASV$vectors$r, 
                        p = fit_ASV$vectors$pvals)

save(fit_ASV, file = paste0(path_out, "/231202_Sugi_phyllosphere_June_16S/analysis/envfit_bray_ASV_BVOC.Rdata"))
write.csv(df_fit_ASV, file = paste0(path_out, "/231202_Sugi_phyllosphere_June_16S/analysis/envfit_bray_ASV_BVOC.csv"))
```
...
ASV_score <- data.frame(ASV_score, 
                        class = fit_ASV$tax_table[ASV_score$rowname, "Phylum"], 
                        p = fit_ASV$vectors$pvals, 
...
show(p_bray)
ggsave(plot = p_bray, filename = paste0(path_out, "/231202_Sugi_phyllosphere_June_16S/figures/envfit_BVOC_ASV_log.png"), 
