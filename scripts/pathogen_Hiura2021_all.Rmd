...
Analyze the abundance patterns of all fungal pathogens described to host Sugi in Kobayashi, 2007.
...
#read the presence-absence data on all described fungal pathogens (Kobayashi, 2007; Hiura et al. 2021)
pathogen <- read.csv(file = paste0(path_out, "/231202_Sugi_phyllosphere_June_ITS/processed_data/pathogens_Hiura2021.csv"))
pathogen <- column_to_rownames(pathogen, var = "Prefecture")

##process the data format
##omit columns with NA
pathogen <- pathogen %>% select_if(function(x) !all(is.na(x)))
...
```{r}
#all pathogen list
path_ls <- data.frame(sciname = colnames(pathogen))

#extract Genus and Species name
path_ls <- path_ls %>%
  mutate(Genus = sapply(str_split(sciname, "\\.", 2), `[`, 1),
         species = sciname, 
         annotation = sapply(str_split(sciname, "\\.", 3), `[`, 3))
#substitute "." in species name with "_"
path_ls$species <- sub("\\.", "_", path_ls$species)
#omit needless information from the column "species"
path_ls$species <- sapply(str_split(path_ls$species, "\\."), `[`, 1)
...
sp_in <- sapply(path_ls$species, charmatch, ps_ITS_sp@tax_table[, "Species"], nomatch = NA)
sp_in <- sp_in[!is.na(sp_in)]
path_sp <- path_ls[path_ls$species == names(sp_in), ]
path_sp
#only one species "Sarea_resinae" was detected, whose function is unknown
View(ps_ITS_sp@otu_table[, rownames(ps_ITS_sp@tax_table[sp_in, ])])

apply(ps_ITS_sp@otu_table[, rownames(ps_ITS_sp@tax_table[sp_in, ])], 2, occurrence)
#both species are relatively minor
apply(ps_ITS_sp@otu_table[, rownames(ps_ITS_sp@tax_table[sp_in, ])] > 0, 2, which)
#Leptosphaeria_sp was detected in 0051, and Sarea_resinae was detected in 0070, 0103
...
p_path <- ggplot(data = abund_plot, 
  ...
  geom_line(data = new_data, aes(y = fit), linewidth = 0.35, color = "black") +
  geom_ribbon(data = new_data, 
              aes(ymin = lower, ymax = upper, y = fit),
              alpha = 0.05, linewidth = 0.35, color = "black", fill = "black") +
  geom_point() + 
  theme_classic() + mytheme +
  xlab("Climatic PC1") + ylab("Abundance of the putative pathogenic fungi after rarefaction") + labs(color = "Guild", shape = "Site") + NULL
