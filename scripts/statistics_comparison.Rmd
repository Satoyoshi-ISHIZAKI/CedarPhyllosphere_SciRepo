...
df_rarefied <- read.csv(paste0(path_out, "/231202_Sugi_phyllosphere_June_ITS/processed_data/exclude/df_rarefied_ITS_out.csv"), row.names = 1)
rownames(df_rarefied) <- formatC(as.numeric(rownames(df_rarefied)), width = 4, flag = "0") 
#add 0 to the head of rownames
...
metadata <- read.csv(paste0(path_out, "/231202_Sugi_phyllosphere_June_ITS/processed_data/exclude/metadata_ITS_out.csv"), row.names = 1)
...
#write.csv(model_bray, file = paste0(path_out, "/231202_Sugi_phyllosphere_June_ITS/analysis/PERMANOVA_bray.csv"))
```
...
#write.csv(model_UF, file = paste0(path_out, "/231202_Sugi_phyllosphere_June_ITS/analysis/PERMANOVA_weightedUF.csv"))
```
...
set.seed(789)
disptest_bray <- permutest(disp_bray, permutations = 999)
show(disptest_bray)
plot(disp_bray)

#write.csv(disptest_bray$tab, file = paste0(path_out, "/231202_Sugi_phyllosphere_June_ITS/analysis/PERMDISP_bray.csv"))
```

##UniFrac

```{r}
disp_UniFrac <- betadisper(dist_UF, 
                        metadata$site, 
                        type = "centroid")
disp_UniFrac

set.seed(890)
disptest_UniFrac <- permutest(disp_UniFrac, permutations = 999)
show(disptest_UniFrac)
plot(disp_UniFrac)

#write.csv(disptest_UniFrac$tab, file = paste0(path_out, "/231202_Sugi_phyllosphere_June_ITS/analysis/PERMDISP_weightedUF.csv"))
```
