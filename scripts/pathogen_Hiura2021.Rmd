...
```{r}
path_total <- sample_sums(ps_ITS_path)
...
```{r}
pldata_total <- left_join(rownames_to_column(data.frame(Abundance = path_total), var = "ID"), 
                          rownames_to_column(metadata, var = "ID"), 
                          by = "ID")
...
p_path <- ggplot(data = pldata_total, 
                 mapping = aes(x = PC1_clim, y = Abundance)) + 
  geom_line(data = new_data, aes(y = fit), linewidth = 0.7, color = "black") +
  geom_ribbon(data = new_data, 
              aes(ymin = lower, ymax = upper, y = fit),
              alpha = 0.05, linewidth = 0, color = NA, fill = "black") +
  geom_point() + 
  theme_classic() + mytheme +
  xlab("Climatic PC1") + ylab("Total abundance of cedar-disease-related fungal genera \nafter rarefaction.") + 
  NULL
  
show(p_path)

ggsave(filename = paste0(path_out, "/231202_Sugi_phyllosphere_June_ITS/figures/pathall_climate1.png"), 
       plot = p_path, 
       dpi = 300, width = 160, height = 90, units = "mm")
```
